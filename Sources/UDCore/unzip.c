...
extern unzFile unzReOpen (const char* path, unzFile file)
{
    ...
    fin= (*(((unz_s *)file)->z_filefunc.zopen_file))(((unz_s *)file)->z_filefunc.opaque,
                                                 path,
                                                 ZLIB_FILEFUNC_MODE_READ |
                                                 ZLIB_FILEFUNC_MODE_EXISTING);
    if (fin==NULL)
...
extern voidpf ZEXPORT unzGetFileStream(unzFile file) {
    return ((unz_s *)file)->filestream;
}

extern void ZEXPORT unzSetFileStream(unzFile file, voidpf stream) {
    ((unz_s *)file)->filestream = stream;
}

extern void ZEXPORT unzCopyFileHandle(unzFile dest, unzFile src) {
    memcpy(dest, src, sizeof(unz_s));
}

extern unsigned long ZEXPORT unzGetCurrentFileUncompressedSize(unzFile file) {
    return ((unz_s *)file)->cur_file_info.uncompressed_size;
}
