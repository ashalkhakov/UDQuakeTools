...
void MD5_Transform( unsigned int state[4], unsigned int const in[16] ) {
    ...
    LittleRevBytes( (unsigned int *)( in ), sizeof(unsigned int), 16 );
...
unsigned long MD5_BlockChecksum( const void *data, int length ) {
    NSCAssert(sizeof(unsigned int) == 4, @"MD5 requires 32-bit digest words");
...
int MD5_FileChecksum( const char *path, char digestHex[33] ) {
    ...
    while ( 1 ) {
        ...
        if ( bytesRead > 0 ) {
            MD5_Update( &ctx, buffer, (unsigned int)( bytesRead ) );
        }
